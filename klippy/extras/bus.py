...
class MCU_SPI:
    def __init__(self, mcu, bus, pin, mode, speed, shutdown_seq, sw_pins=None, inverted=0):
        self.mcu = mcu
        self.bus = bus
        ...
        if sw_pins is not None:
            ...
            self.config_fmt = (
                "config_spi oid=%d spi_bus=%%s pin=%s inverted=%u mode=%d rate=%d"
                " shutdown_msg=%s" % (
                    self.oid, pin, inverted, mode, speed, shutdown_msg))
...
def MCU_SPI_from_config(config, mode, pin_option="cs_pin",
    ...
    return MCU_SPI(mcu, bus, pin, mode, speed, shutdown_seq, sw_pins, invert)

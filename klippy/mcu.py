...
class MCU_stepper:
    ...
    def step_const(self, print_time, start_pos, dist, start_v, accel, core=False):
    ...
    def build_config(self):
        ...
        if self._hard_pwm:
            ...
            self._mcu.add_config_cmd(
                ...
                " default_value=%d max_duration=%d" % (
                    self._oid, self._pin,
                    self._mcu.seconds_to_clock(cycle_ticks),
...
class MCU:
    ...
    def do_shutdown(self, force=False): # TODO: remove??
        self._shutdown()
    def disconnect(self): # TODO: remove???
        self._disconnect()
    def __del__(self):
        self.disconnect()

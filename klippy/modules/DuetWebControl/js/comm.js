	...
	$.ajax(ajaxPrefix + ajaxRequest, {
		...
		success: function(status) {
			...
			if (status.hasOwnProperty("endstops")) {
				//var drive_count = $('#table_drives > tbody > tr').length;
			...
			if (status.err == 0 && (lastStatusResponse == undefined || (status.status != 'F' && lastStatusResponse.seq != status.seq))) {
				$.ajax(ajaxPrefix + "rr_reply", {
					...
					success: function(response) {
						...
						if (response != "" || (lastSentGCode != "" && oldResponse == response)) {
							...
							} else if (response.match("Warning: ") != null) {
								style = "warning";
	...
	$.ajax(ajaxPrefix + "rr_config", {
		...
		success: function(response) {
			...
			*/
			...
			var rowCount = $('#table_drives > tbody > tr').length;
			if (rowCount < response.accelerations.length) {
				// Add missing drives
				for (var drive = rowCount; drive < response.accelerations.length; drive++) {
					$('#table_drives > tbody:last-child').append(

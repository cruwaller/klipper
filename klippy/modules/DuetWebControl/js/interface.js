...
$("#panel_offset label.btn").click(function() {
			...
			if (heater == "bed") {
				...
				if (hasToolSelected) {
					...
					if (lastStatusResponse.temps.state[heater] == 1) {
						...
						var sParam = active.join(function(a, b) { return a + ":" + b; });
						var rParam = standby.join(function(a, b) { return a + ":" + b; });
						sendGCode("M104 P" + tools[0] + "S" + sParam + " R" + rParam);	// Turn off associated heater
					} else {
...
function setBoardType(type) {
	...
	var isWiFi, isDuetNG, isDuetMaestro, update = true;
...
function setProbeValue(value, secondaryValue) {
	if (value == undefined) {
		...
		$("#td_probe, #dd_probe_current_value").html("z: " + value);
	} else {
		//$("#td_probe, #dd_probe_current_value").html(value + " (" + secondaryValue.reduce(function(a, b) { return a + b; }) + ")");
		$("#td_probe, #dd_probe_current_value").html("z: " + value + " x: " + secondaryValue[0] + " y: " + secondaryValue[1]);
	}
